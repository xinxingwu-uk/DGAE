...
#--------------------------------------------------------------------------------------------------------------------
class OptimizerVAE_FeatureReconstrution(object):
    def __init__(self,preds_adj,preds_Features,labels_adj,labels_Features,labels_adj_rec,\
                 loss_coeff_adj,loss_coeff_AE,model,num_nodes,pos_weight,norm):
        ...
        rec_features_prediction =tf.square(tf.subtract(preds_sub_Features,\
                                                       tf.sparse_tensor_dense_matmul(labels_adj_rec,tf.sparse.to_dense(labels_sub_Features))))   
        ...
        self.cost -= loss_coeff_adj*self.kl        
...
#--------------------------------------------------------------------------------------------------------------------
class OptimizerVAE_AdjReconstrution(object):
    def __init__(self, preds_adj,preds_adj_rec,labels_adj,labels_Features,labels_adj_rec,\
                 loss_coeff_adj,loss_coeff_AE,model,num_nodes,pos_weight,norm):
        preds_sub_adj = preds_adj
        labels_sub_adj = labels_adj
        preds_sub_adj_rec=preds_adj_rec
        labels_sub_adj_rec=labels_adj_rec
        rec_adj_prediction = tf.square(tf.subtract(preds_sub_adj_rec,\
                                                   tf.sparse_tensor_dense_matmul(labels_sub_adj_rec,tf.sparse.to_dense(labels_Features))))     
