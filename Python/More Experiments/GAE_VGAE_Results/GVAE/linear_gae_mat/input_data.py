...
def load_mat_data(dataset_str):
    """ dataset_str: protein, metabolic, conflict """
    dataset_path = '/work/07913/xwu236/GVAE/Datasets/' + dataset_str + '.mat'
...
def load_data(dataset):
    ...
    for i in range(len(names)):
        with open("/work/07913/xwu236/GVAE/Datasets/ind.{}.{}".format(dataset, names[i]), 'rb') as f:
            if sys.version_info > (3, 0):
                objects.append(pkl.load(f, encoding='latin1'))
            else:
                objects.append(pkl.load(f))
    x, tx, allx, graph = tuple(objects)
    test_idx_reorder = parse_index_file("/work/07913/xwu236/GVAE/Datasets/ind.{}.test.index".format(dataset))
