...
from linear_gae_mat.optimizer import OptimizerAE, OptimizerVAE
...
#--------------------------------------------------------------------------------------------------------------------
# Load data
adj_, features_ = load_mat_data(dataset)
...
for seed_i in np.arange(nb_run):
    ...
    if model_name == 'gcn_ae':
        ...
        model = LinearModelVAE(placeholders, num_features, num_nodes,
                               features_nonzero)
    elif model_name == 'deep36_gcn_ae':
        # Deep (3-layer GCN) Graph Autoencoder
        model = Deep36GCNModelAE(placeholders, num_features, features_nonzero)
    ...
    with tf.name_scope('optimizer'):
        ...
        if model_name in ('gcn_ae', 'linear_ae', 'deep_gcn_ae','deep36_gcn_ae'):
        ...
        elif model_name in ('gcn_vae', 'linear_vae', 'deep_gcn_vae','deep36_gcn_vae'):
...
write_to_csv(mean_time_.reshape(1,len(mean_time_)),"/home1/07913/xwu236/GAE/8Conflict_AE/log/"+p_model_name+"_time_"+str(features_used)+".csv")

mean_roc_=np.array(mean_roc)
write_to_csv(mean_roc_.reshape(1,len(mean_roc_)),"/home1/07913/xwu236/GAE/8Conflict_AE/log/"+p_model_name+"_roc_"+str(features_used)+".csv")

mean_ap_=np.array(mean_ap)
write_to_csv(mean_ap_.reshape(1,len(mean_ap_)),"/home1/07913/xwu236/GAE/8Conflict_AE/log/"+p_model_name+"_ap_"+str(features_used)+".csv")
